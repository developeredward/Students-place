...
const FeedDetail = ({ feeds, isAuthenticated, token, userImage }) => {
  const { id } = useParams();
  const initialState = {
    data: "",
    comment: "",
  ...
  const {
    ...
    updateFeed: updated,
    error: resourceError,
  } = useFetch(token, `http://127.0.0.1:8000/api/feeds/${id}`);
  ...
  return (
    <div className=" feeds-detail feeds-container">
      <div className="feed">
        {loading ? (
          <div>
            {" "}
                ...
                <ReturnNavBar
                  ...
                  text={
                    feed.post_by.middle_name ? (
                      <h3>
                        {feed.post_by.first_name +
                          " " +
                          feed.post_by.middle_name +
                          " " +
                          feed.post_by.last_name}
                        's Post
                      </h3>
                    ) : (
                      <h3>
                        {feed.post_by.first_name + " " + feed.post_by.last_name}
                        's Post
                      </h3>
                    )
                  }
                />
                <div className="post" name="postId" key={feed.id.toString()}>
                  <div className="post-user">
                    <Link
                      style={{ textDecoration: "none", color: "unset" }}
                      to={`/${feed.post_by.username}`}
                    >
                      <div className="user">
                        <div className="profile-pic">
                          {feed.post_by.profile_picture ? (
                            <img
                              className="img"
                              src={feed.post_by.profile_picture}
                              alt=""
                            />
                          ) : (
                            <FontAwesomeIcon
                        ...
                        <div className="user-details">
                          <div className="user-fullname-username">
                            <div className="fullname">
                              <h3>{feed.post_by.get_full_name}</h3>
                              {feed.post_by.verified ? (
                            ...
                            <div className="username">
                              <span>@{feed.post_by.username}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="timestamp-options">
                      ...
                      <div className="timestamp">
                        <span>
                          {new Date(feed.timestamp).toLocaleString("en-US", {
                            weekday: "short",
                            day: "numeric",
                            month: "long",
                            year: "2-digit",
